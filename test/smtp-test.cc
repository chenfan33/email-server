...
int main(int argc, char *argv[])
{
  ...
  writeString(&conn1, "MAIL FROM:<cfan@localhost>\r\n");
  expectToRead(&conn1, "250 OK");
  expectNoMoreData(&conn1);
  
  writeString(&conn1, "HELO\r\n");
  expectToRead(&conn1, "500 *");
  expectNoMoreData(&conn1);
  ...
  writeString(&conn1, "RCPT TO:<non@localhost>\r\n");
